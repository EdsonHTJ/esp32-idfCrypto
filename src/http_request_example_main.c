...
esp_err_t _http_event_handler(esp_http_client_event_t *evt)
{
    ...
    switch(evt->event_id) {
        ...
        case HTTP_EVENT_ON_DATA:
            ...
            ESP_LOGI( TAG,"CONTENT %s", (char *) evt->data);
...
static void https_with_hostname_path(void)
{
    ...
    esp_http_client_config_t config = {
        ...
        .url = "https://api.trongrid.io/v1/accounts/TWsFJR5PPBa96PkNAPzKB6aLtvKpiP31na",
    ...
    esp_http_client_handle_t client = esp_http_client_init(&config);
    esp_http_client_set_url(client, "https://api.trongrid.io/v1/accounts/TWsFJR5PPBa96PkNAPzKB6aLtvKpiP31na");
    ...
    if (err == ESP_OK) {
        ...
        int len = -1;
        if(esp_http_client_get_chunk_length(client, &len) != ESP_OK){
            ESP_LOGI(TAG, "ERROR");
        }
        if(esp_http_client_flush_response(client, &len) != ESP_OK){
             ESP_LOGI(TAG, "ERROR2");
        }
        int len2 =  esp_http_client_read_response(client, buff, 8000);
        ESP_LOGI(TAG, "contentLen: %i , content: %s", len,buff);
    } else {
        ESP_LOGE(TAG, "HTTP HEAD request failed: %s", esp_err_to_name(err));
    }
...
static void http_test_task(void *pvParameters){
    ...
    ESP_LOGI(TAG, "%s", server_root_cert_pem_start);
    while (1)
    {
...
void app_main(void)
{
    ...
    xTaskCreate(&http_test_task, "http_get_task", 4 * 8192, NULL, 5, NULL);
}
